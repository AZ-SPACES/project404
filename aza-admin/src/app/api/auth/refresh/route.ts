 ...
 * the refresh token. The refresh token is read server-side from the cookie and is
 * never exposed to the browser; only the new access token is returned to JS memory.
 */
export async function POST(request: NextRequest) {
  ...
  if (!upstream.ok || !body?.success || !body?.data?.accessToken) {
    // Refresh token is invalid/expired — clear it so the client falls back to login.
