...
function ComposeModal({ onClose }: { onClose: () => void }) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-card border border-border rounded-2xl p-8 w-full max-w-sm shadow-2xl text-center">
          ...
          <h3 className="text-lg font-semibold text-foreground mb-2">Notification Sent</h3>
          <p className="text-foreground/50 text-sm mb-6">
            Delivered to{" "}
            <span className="text-foreground font-semibold">{sent.toLocaleString()}</span>{" "}
            user{sent !== 1 ? "s" : ""}
          </p>
          <button
            onClick={onClose}
        ...
        <div className="relative bg-card border border-border rounded-2xl p-6 w-full max-w-sm shadow-2xl">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-foreground/40">This cannot be undone</p>
            </div>
          </div>

          <div className="bg-white/4 border border-border rounded-xl p-4 mb-5 space-y-2.5 text-sm">
            <div className="flex justify-between gap-2">
              <span className="text-foreground/40 flex-shrink-0">Audience</span>
              <span className="text-foreground font-medium text-right">{audienceLabel}</span>
            </div>
            <div className="flex justify-between gap-2">
              <span className="text-foreground/40 flex-shrink-0">Title</span>
              <span className="text-foreground font-medium text-right truncate max-w-[200px]">{title}</span>
            </div>
            <div className="flex justify-between gap-2">
              <span className="text-foreground/40 flex-shrink-0">Message</span>
              <span className="text-foreground/70 text-right text-xs leading-relaxed max-w-[200px] line-clamp-3">{body}</span>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 py-2.5 rounded-xl bg-muted/30 border border-border text-foreground/60 hover:text-foreground text-sm font-medium transition-all disabled:opacity-50"
            >
              Cancel
            </button>
            <button
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/70" onClick={onClose} />
      <div className="relative bg-card border border-border rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          <h2 className="text-base font-semibold text-foreground">Compose Broadcast</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          {/* Audience */}
          <div>
            <label className="block text-xs font-semibold text-foreground/40 uppercase tracking-wider mb-3">
              Audience
            </label>
              ...
              {AUDIENCES.map((a) => (
                <button
                  ...
                  className={`flex items-center gap-2 px-3.5 py-2 rounded-xl border text-sm font-medium transition-all ${
                    audience === a.value
                      ? "bg-[#B7EE7A]/10 border-[#B7EE7A]/40 text-foreground"
                      : "bg-white/3 border-border text-foreground/50 hover:text-foreground hover:border-border"
                  }`}
                >
                  <span className={audience === a.value ? "text-[#B7EE7A]" : "text-foreground/30"}>
                    {a.icon}
                  </span>
                  {a.label}
                  <span
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span
                    className={`text-xs ${
                      title.length >= TITLE_MAX ? "text-amber-400" : "text-foreground/30"
                    }`}
                  >
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl bg-muted/30 border border-border text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-[#B7EE7A]/40 text-sm transition-colors"
                />
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span
                    className={`text-xs ${
                      body.length >= BODY_MAX ? "text-amber-400" : "text-foreground/30"
                    }`}
                  >
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-muted/30 border border-border text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-[#B7EE7A]/40 text-sm resize-none transition-colors"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-foreground/40 uppercase tracking-wider mb-1.5">
                  Image URL{" "}
                  <span className="normal-case font-normal text-foreground/25">(optional)</span>
                </label>
                <input
                  type="url"
                  value={imageUrl}
                ...
                <div className="bg-card rounded-2xl p-3.5 shadow-lg border border-border">
                  <div className="flex items-center gap-2 mb-2.5">
                    ...
                    <div className="flex items-center gap-1.5 flex-1">
                      <span className="text-[11px] font-semibold text-foreground/60">aza</span>
                      <span className="text-[10px] text-foreground/30">· now</span>
                    </div>
                  </div>
                  {imageUrl && (
                    <div className="mb-2.5 w-full h-24 rounded-lg bg-black/20 overflow-hidden border border-border">
                      <img
                        src={imageUrl}
                  ...
                  <p className="text-sm font-semibold text-foreground leading-snug mb-0.5">
                    {title || (
                      <span className="text-foreground/20 font-normal">Notification title</span>
                    )}
                  </p>
                  <p className="text-xs text-foreground/50 leading-relaxed">
                    {body || (
                      <span className="text-foreground/20">Message body here…</span>
                    )}
                  </p>
                </div>
              </div>
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        {[
          ...
          <div key={card.label} className="bg-card border border-border rounded-xl p-4">
            ...
            <p className="text-xs text-foreground/40 mt-0.5 leading-snug">{card.description}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground">Push Notification Broadcasts</p>
