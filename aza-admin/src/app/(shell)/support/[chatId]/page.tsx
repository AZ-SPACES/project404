  ...
  return (
    <div className="p-4 border-b border-border">
      <div className="flex items-center justify-between mb-2">
        <span className="text-[10px] font-semibold uppercase tracking-widest text-foreground/30">SLA</span>
...
function InfoRow({ label, value, valueClass = "text-foreground/70" }: { label: string; value: string; valueClass?: string }) {
  return (
    <div className="flex items-center justify-between gap-2">
      <span className="text-[11px] text-foreground/35 flex-shrink-0">{label}</span>
...
function CustomerPanel({ user, chat, recentTxns }: { user: AdminUser | null; chat: SupportChatSummary | null; recentTxns: AdminTransaction[] }) {
  if (!user) {
    return (
      <div className="p-5">
        <div className="w-12 h-12 rounded-full bg-muted/40 mx-auto mb-3 animate-pulse" />
        <div className="space-y-2">
          {[...Array(4)].map((_, i) => <div key={i} className="h-3 bg-muted/30 rounded animate-pulse" />)}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="divide-y divide-border">
      <div className="p-5 text-center">
        <div className="w-14 h-14 rounded-full bg-muted/40 flex items-center justify-center overflow-hidden mx-auto mb-3 border border-border">
          {user.profileImageUrl ? (
            <img src={user.profileImageUrl} alt="" className="w-full h-full object-cover" />
          ) : (
            <span className="text-xl font-bold text-foreground/50">{user.firstName?.charAt(0)?.toUpperCase()}</span>
          )}
        </div>
        <p className="text-sm font-semibold text-foreground">{[user.firstName, user.lastName].filter(Boolean).join(" ") || user.username}</p>
        {user.username && <p className="text-xs text-foreground/40 mt-0.5">@{user.username}</p>}
        <p className="text-xs text-foreground/30 mt-0.5 truncate">{user.email}</p>
      </div>
      ...
      {recentTxns.length > 0 && (
        <div className="px-5 py-4">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-foreground/30 mb-3">Recent Transactions</p>
          <div className="space-y-2">
            {recentTxns.slice(0, 4).map((tx) => (
              <div key={tx.id} className="flex items-center justify-between">
                <div className="min-w-0">
                  <p className="text-xs text-foreground/70 truncate">{tx.type}</p>
                  <p className="text-[10px] text-foreground/30">{timeAgo(tx.initiatedAt)}</p>
                </div>
...
function CannedPanel({ responses, onSelect }: { responses: CannedResponse[]; onSelect: (text: string) => void }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-border">
        <p className="text-[10px] font-semibold uppercase tracking-widest text-foreground/30 mb-3">Quick Replies</p>
        <div className="relative">
          <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-foreground/25" />
          <input
            type="text"
            placeholder="Search templates..."
            value={q}
            onChange={(e) => setQ(e.target.value)}
            className="w-full pl-7 pr-3 py-1.5 bg-muted/30 border border-border rounded-lg text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-foreground/20"
          />
        </div>
      </div>
      <div className="flex-1 overflow-y-auto">
        {Object.entries(grouped).map(([cat, items]) => (
          <div key={cat}>
            <p className="px-4 py-2 text-[9px] font-bold uppercase tracking-widest text-foreground/20 bg-muted/10">{cat}</p>
            {items.map((r) => (
              <button
                key={r.id}
                onClick={() => onSelect(r.content)}
                className="w-full text-left px-4 py-3 hover:bg-muted/30 border-b border-border transition-colors group"
              >
                <div className="flex items-center justify-between mb-0.5">
                  <p className="text-xs font-semibold text-foreground/80 group-hover:text-foreground transition-colors">{r.title}</p>
                  <Zap size={11} className="text-[#B7EE7A] opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
                </div>
                <p className="text-[11px] text-foreground/35 line-clamp-2 leading-relaxed">{r.content}</p>
              </button>
            ))}
          </div>
        ))}
        {filtered.length === 0 && (
          <div className="py-8 text-center text-foreground/25 text-xs">No templates found</div>
        )}
      </div>
    </div>
  );
...
export default function SupportChatPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] lg:h-[calc(100vh-0rem)] max-w-[1440px] mx-auto -mx-6 lg:-mx-8 -my-6 lg:-my-8 overflow-hidden">
      ...
      <>
        <div className="hidden xl:flex flex-col w-72 border-r border-border bg-background overflow-y-auto flex-shrink-0">
          <div className="px-5 pt-5 pb-3 border-b border-border flex-shrink-0">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-foreground/30">Customer Profile</p>
          </div>
        ...
        {showLeftPanel && (
          <>
            ...
            <div className="fixed inset-y-0 left-0 z-50 w-72 bg-background border-r border-border overflow-y-auto xl:hidden">
              <div className="px-5 pt-5 pb-3 border-b border-border flex items-center justify-between">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-foreground/30">Customer Profile</p>
                <button onClick={() => setShowLeftPanel(false)} className="text-foreground/40 hover:text-foreground"><X size={16} /></button>
              </div>
              <CustomerPanel user={chatUser ?? null} chat={chat} recentTxns={recentTxns} />
            </div>
          </>
      ...
      <div className="flex-1 flex flex-col min-h-0 min-w-0 bg-background">
        ...
        {chat && !isResolved && (
          chat.botActive ? (
            ...
            <div className="flex items-center justify-between gap-3 px-4 py-2 bg-muted/10 border-b border-border flex-shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-lg bg-muted/30 border border-border text-foreground/60 hover:text-foreground hover:bg-muted transition-colors disabled:opacity-50"
              >
                <Cpu size={12} />
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-card flex-shrink-0">
          <button
            onClick={() => router.back()}
            className="p-1.5 rounded-lg hover:bg-muted/50 text-foreground/40 hover:text-foreground transition-all flex-shrink-0"
          >
            <ArrowLeft size={18} />
          </button>
          ...
          <button
            ...
            className="xl:hidden p-1.5 rounded-lg hover:bg-muted/50 text-foreground/30 hover:text-foreground transition-all flex-shrink-0"
          >
            <PanelLeftOpen size={16} />
          </button>

          <div className="flex items-center gap-2.5 flex-1 min-w-0">
            <div className="w-8 h-8 rounded-full bg-muted/40 flex items-center justify-center overflow-hidden flex-shrink-0 border border-border">
              {chat?.userAvatar
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-foreground leading-tight truncate">
                ...
                {chat?.userHandle && <span className="text-foreground/35 font-normal ml-1.5 text-xs">@{chat.userHandle}</span>}
              </p>
              {chat?.category && <span className="text-[10px] text-foreground/35">{chat.category}</span>}
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            <div className="relative">
              <button
                ...
                className="hidden sm:flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-muted/30 hover:bg-muted/40 border border-border text-xs text-foreground/60 transition-all"
              >
                {chat?.category ?? "Category"}
                <ChevronDown size={12} />
              </button>
              {categoryOpen && (
                <div className="absolute right-0 top-full mt-1 z-30 bg-card border border-border rounded-xl shadow-2xl overflow-hidden min-w-[140px]">
                  {CATEGORIES.map((c) => (
                    <button
                      key={c}
                      onClick={() => categoryMutation.mutate(c)}
                      className={`w-full text-left px-4 py-2 text-xs transition-colors ${
                        chat?.category === c ? "text-[#B7EE7A] bg-[#B7EE7A]/10" : "text-foreground/60 hover:text-foreground hover:bg-muted/50"
                      }`}
                    >
            ...
            <div className="relative">
              <button
                ...
                className={`hidden sm:flex items-center gap-1 px-2.5 py-1.5 rounded-lg border text-xs font-semibold transition-all ${
                  ...
                  "bg-muted/30 border-border text-foreground/50"
                }`}
              >
                {chat?.priority ?? "—"}
                <ChevronDown size={12} />
              ...
              {priorityOpen && (
                <div className="absolute right-0 top-full mt-1 z-30 bg-card border border-border rounded-xl shadow-2xl overflow-hidden min-w-[120px]">
                  {PRIORITIES.map((p) => (
                    <button
                      key={p}
                      onClick={() => priorityMutation.mutate(p)}
                      className={`w-full text-left px-4 py-2 text-xs transition-colors ${
                        chat?.priority === p ? "text-[#B7EE7A] bg-[#B7EE7A]/10" : "text-foreground/60 hover:text-foreground hover:bg-muted/50"
                      }`}
                    >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all border ${
                isResolved
                  ? "bg-muted/30 border-border text-foreground/50 hover:bg-muted hover:text-foreground"
            ...
            <button
              ...
              className="lg:hidden p-1.5 rounded-lg hover:bg-muted/50 text-foreground/30 hover:text-foreground transition-all"
            >
        ...
        >
            ...
            <div className="space-y-4 pt-4">
              {[...Array(5)].map((_, i) => (
                <div key={i} className={`flex ${i % 2 === 1 ? "justify-end" : "justify-start"}`}>
                  <div className={`h-10 rounded-2xl bg-muted/30 animate-pulse ${i % 2 === 1 ? "w-40" : "w-56"}`} />
                </div>
              ))}
            </div>
          ) : chatItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-16">
              <MessageSquare size={32} className="text-foreground/15 mb-3" />
              <p className="text-sm text-foreground/30 font-medium">No messages yet</p>
              <p className="text-xs text-foreground/20 mt-1">Start the conversation below</p>
            </div>
          ) : (
            chatItems.map((item) => {
              if (item.kind === "note") {
                  ...
                  <div className={`flex flex-col ${isAgent ? "items-end" : "items-start"} max-w-[80%] lg:max-w-[72%]`}>
                    ...
                    <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                      isAgent
                        ...
                        : isBot
                          ? "bg-[#B7EE7A]/8 border border-[#B7EE7A]/15 text-foreground/85 rounded-bl-sm"
                          : "bg-card text-foreground/85 border border-border rounded-bl-sm"
                    }`}>
                      {msg.isDeleted
                    ...
                    <span className={`text-[10px] mt-1 text-foreground/25 ${isAgent ? "mr-1" : "ml-1"}`}>
                      {fmtTime(msg.sentAt)}
                    </span>
                  </div>
                </div>
          ...
          {isOtherTyping && (
            <div className="flex justify-start">
              <div className="flex items-center gap-2 bg-card border border-border px-4 py-2.5 rounded-2xl rounded-bl-sm">
                ...
                <span className="text-[10px] text-foreground/35 font-medium uppercase tracking-wider">
        ...
        {/* Input area */}
        <div className="border-t border-border bg-card px-4 pt-3 pb-4 flex-shrink-0">
          {error && (
          ...
          <div className={`flex items-end gap-2 rounded-2xl p-2 border transition-all ${
            inputMode === "note"
              ...
              : "bg-card border-border focus-within:border-foreground/20"
          }`}>
            <textarea
              ref={textareaRef}
              rows={1}
              ...
              className={`flex-1 bg-transparent px-2 py-1.5 text-sm resize-none focus:ring-0 border-none min-h-[38px] max-h-40 ${
                inputMode === "note" ? "text-amber-200/70 placeholder-amber-400/25" : "text-foreground placeholder:text-foreground/20"
              } disabled:opacity-40`}
            />
            <button
              onClick={handleSend}
          ...
          <div className="mt-2 flex justify-between items-center px-1">
            <p className="text-[10px] text-foreground/20 uppercase tracking-widest">Enter to send · Shift+Enter for newline</p>
            <div className="flex items-center gap-1.5">
              <div className={`w-1.5 h-1.5 rounded-full ${isConnected ? "bg-emerald-500" : "bg-foreground/20"}`} />
              <span className="text-[10px] text-foreground/20 uppercase tracking-widest">{isConnected ? "Live" : "Connecting"}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Right Panel – Tools */}
      <>
        <div className="hidden lg:flex flex-col w-72 border-l border-border bg-card flex-shrink-0 overflow-hidden">
        ...
        {showRightPanel && (
          <>
            ...
            <div className="fixed inset-y-0 right-0 z-50 w-72 bg-card border-l border-border flex flex-col overflow-hidden lg:hidden">
              <div className="px-4 py-3.5 border-b border-border flex items-center justify-between flex-shrink-0">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-foreground/30">Tools</p>
                <button onClick={() => setShowRightPanel(false)} className="text-foreground/40 hover:text-foreground"><X size={16} /></button>
              </div>
              <SlaIndicator priority={chat?.priority ?? "NORMAL"} lastMessageAt={chat?.lastMessageAt ?? null} status={chat?.status ?? "OPEN"} />
              <div className="flex-1 overflow-hidden">
