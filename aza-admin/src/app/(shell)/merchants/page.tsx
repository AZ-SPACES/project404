...
function StatusBadge({ status }: { status: string }) {
  const cfg = STATUS_CFG[status] ?? { cls: "text-foreground/40 bg-muted/30 border-border", label: status };
  return (
...
function MerchantAvatar({ merchant }: { merchant: AdminMerchant }) {
  ...
  const initials = merchant.businessName
    ...
    .toUpperCase();
  return (
    <div className="w-8 h-8 rounded-lg bg-muted/40 border border-border flex items-center justify-center flex-shrink-0">
      <span className="text-[10px] font-bold text-foreground/50">{initials}</span>
    </div>
  );
}
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-foreground/40 text-sm mt-0.5">Business accounts, KYB review, and status management</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: "Total", value: stats?.total ?? data?.totalElements ?? "—", color: "text-foreground", icon: Store },
          { label: "Active", value: stats?.active ?? "—", color: "text-emerald-400", icon: Store },
          { label: "Awaiting KYB", value: reviewQueue || "—", color: reviewQueue > 0 ? "text-amber-400" : "text-foreground/40", icon: ShieldCheck },
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="bg-card border border-border rounded-xl px-4 py-3 flex items-center justify-between">
            <div>
              <p className="text-[10px] text-foreground/30 uppercase tracking-wider font-medium mb-0.5">Total Merchant Balances</p>
              <p className="text-lg font-semibold text-foreground">{fmtAmount(stats.totalBalance)}</p>
            </div>
            <DollarSign size={18} className="text-foreground/15" />
          </div>
          <div className="bg-card border border-border rounded-xl px-4 py-3 flex items-center justify-between">
            <div>
              <p className="text-[10px] text-foreground/30 uppercase tracking-wider font-medium mb-0.5">Suspended / Rejected</p>
              <p className="text-lg font-semibold text-red-400">{(stats.suspended + stats.rejected).toLocaleString()}</p>
            </div>
            <AlertTriangle size={18} className="text-foreground/15" />
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-muted/30 border border-border rounded-xl text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-foreground/20"
          />
        </div>
          ...
          {STATUS_FILTERS.slice(0, 5).map((s) => (
            <button
              ...
              className={`relative px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                statusFilter === s ? "bg-[#B7EE7A] text-black" : "text-foreground/50 hover:text-foreground"
              }`}
            >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left px-5 py-3 text-[11px] font-semibold text-foreground/30 uppercase tracking-wider">Business</th>
                <th className="text-left px-5 py-3 text-[11px] font-semibold text-foreground/30 uppercase tracking-wider hidden md:table-cell">Category</th>
                <th className="text-left px-5 py-3 text-[11px] font-semibold text-foreground/30 uppercase tracking-wider">Status</th>
                <th className="text-right px-5 py-3 text-[11px] font-semibold text-foreground/30 uppercase tracking-wider hidden lg:table-cell">Balance</th>
                <th className="text-right px-5 py-3 text-[11px] font-semibold text-foreground/30 uppercase tracking-wider hidden lg:table-cell">Volume</th>
                <th className="text-right px-5 py-3 text-[11px] font-semibold text-foreground/30 uppercase tracking-wider hidden sm:table-cell">Created</th>
                ...
                >
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <p className="font-semibold text-foreground leading-tight">{m.businessName}</p>
                        <p className="text-xs text-foreground/35 font-mono">@{m.businessHandle}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-right hidden lg:table-cell">
                    <span className="text-foreground/70 font-mono text-xs">{fmtAmount(m.balance, m.currency)}</span>
                  </td>
                  <td className="px-5 py-3.5 text-right hidden lg:table-cell">
                    <span className="text-foreground/50 font-mono text-xs">{fmtAmount(m.totalVolume, m.currency)}</span>
                  </td>
                  <td className="px-5 py-3.5 text-right hidden sm:table-cell">
                    <span className="text-foreground/35 text-xs">{fmtDate(m.createdAt)}</span>
                  </td>
                  <td className="px-3 py-3.5">
                    <ChevronRight size={16} className="text-foreground/20 group-hover:text-foreground/50 transition-colors ml-auto" />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {data && data.totalPages > 1 && (
        <div className="flex justify-center items-center gap-3">
          <button
            ...
            className="px-4 py-2 text-sm rounded-xl bg-muted/30 hover:bg-muted disabled:opacity-30 border border-border"
          >
            Previous
          </button>
