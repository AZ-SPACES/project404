  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-white/40 text-sm mt-0.5">Platform fee schedule and revenue configuration</p>
      </div>
      ...
      {rules.length === 0 ? (
        <div className="text-center py-20 text-white/25">
          ...
          <p className="text-xs mt-1 text-white/20">Fee rules will appear here once the backend API is connected.</p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/5">
                <th className="text-left px-5 py-3 text-[10px] font-semibold uppercase tracking-widest text-white/30">Rule</th>
                <th className="text-left px-5 py-3 text-[10px] font-semibold uppercase tracking-widest text-white/30 hidden sm:table-cell">Transaction Type</th>
                <th className="text-left px-5 py-3 text-[10px] font-semibold uppercase tracking-widest text-white/30">Type</th>
                <th className="text-right px-5 py-3 text-[10px] font-semibold uppercase tracking-widest text-white/30">Amount</th>
                <th className="text-center px-5 py-3 text-[10px] font-semibold uppercase tracking-widest text-white/30 hidden md:table-cell">Tier</th>
                <th className="text-center px-5 py-3 text-[10px] font-semibold uppercase tracking-widest text-white/30">Status</th>
              ...
              {rules.map((rule) => (
                <tr key={rule.id} className="hover:bg-white/2 transition-colors">
                  <td className="px-5 py-4">
                    ...
                    {rule.description && <p className="text-xs text-white/35 mt-0.5">{rule.description}</p>}
                  </td>
                  <td className="px-5 py-4 hidden sm:table-cell">
                    <span className="text-xs text-white/55 px-2 py-1 rounded bg-white/5 border border-white/8">
                  ...
                  <td className="px-5 py-4 text-center hidden md:table-cell">
                    {rule.tierMinAmount !== null || rule.tierMaxAmount !== null ? (
                      ...
                      <span className="text-xs text-white/20">All amounts</span>
                    )}
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#1a1a1a] border border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-base font-semibold text-white">Edit Fee Rule</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/8 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-white/20"
                />
              </div>
              ...
              {editing.feeType === "PERCENTAGE" && (
                <>
                  <div>
                    <label className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-2 block">Min Fee (GHS)</label>
                    <input
                      type="number"
                      value={editDraft.minFee ?? ""}
                      onChange={(e) => setEditDraft((d) => ({ ...d, minFee: e.target.value ? Number(e.target.value) : null }))}
                      placeholder="No minimum"
                      className="w-full bg-white/5 border border-white/8 rounded-xl px-3 py-2 text-sm text-white placeholder-white/20 focus:outline-none focus:border-white/20"
                    />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-2 block">Max Fee (GHS)</label>
                    <input
                      type="number"
