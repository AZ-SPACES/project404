...
const STATUS_STYLE: Record<string, string> = {
  ...
  PENDING: "bg-muted/50 text-foreground/40",
};
      ...
      <div className="relative bg-card border border-border rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          <div>
            ...
            <p className="text-sm font-semibold text-foreground mt-0.5">Bulk Transfer Detail</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="flex items-center justify-center h-48"><p className="text-foreground/40 text-sm">Failed to load</p></div>
        ) : (
          <div className="overflow-y-auto flex-1">
            <div className="px-6 py-5 grid grid-cols-3 gap-4 border-b border-border">
              <div><p className="text-xs text-foreground/30 mb-1">Total</p><p className="text-base font-bold text-foreground">{fmt(detail.totalAmount)}</p></div>
              <div><p className="text-xs text-foreground/30 mb-1">Succeeded</p><p className="text-base font-bold text-[#B7EE7A]">{detail.successCount}</p></div>
              <div><p className="text-xs text-foreground/30 mb-1">Failed</p><p className="text-base font-bold text-red-400">{detail.failureCount}</p></div>
            </div>
            <div className="px-6 py-4 space-y-2 text-sm border-b border-border">
              {detail.note && <div className="flex justify-between"><span className="text-foreground/35">Note</span><span className="text-foreground/70">{detail.note}</span></div>}
              <div className="flex justify-between"><span className="text-foreground/35">Status</span><span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${STATUS_STYLE[detail.status] ?? ""}`}>{detail.status.replace(/_/g, " ")}</span></div>
              <div className="flex justify-between"><span className="text-foreground/35">Processed</span><span className="text-foreground/70">{fmtDate(detail.processedAt)}</span></div>
            </div>
              ...
              {detail.items.map((item) => {
                ...
                return (
                  <div key={item.id} className="px-6 py-3 flex items-center gap-3">
                    <Icon size={14} className={item.status === "COMPLETED" ? "text-[#B7EE7A]" : item.status === "FAILED" ? "text-red-400" : "text-foreground/30"} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-foreground truncate">{item.recipientIdentifier}</p>
                      {item.failureReason && <p className="text-xs text-red-400/70 mt-0.5">{item.failureReason}</p>}
                      {item.note && <p className="text-xs text-foreground/30 mt-0.5">{item.note}</p>}
                    </div>
                    <span className="text-sm font-medium text-foreground flex-shrink-0">{fmt(item.amount)}</span>
                  </div>
                );
              })}
            </div>
...
function CreateModal({ onClose, onCreate }: { onClose: () => void; onCreate: (bt: BulkTransfer) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-card border border-border rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          <h2 className="text-base font-bold text-foreground">New Bulk Transfer</h2>
        ...
        <form onSubmit={submit} className="flex flex-col flex-1 overflow-hidden">
          <div className="px-6 py-4 border-b border-border flex-shrink-0">
            <label className="block text-xs text-foreground/40 mb-1.5">Batch note <span className="text-foreground/20">(optional)</span></label>
            <input
              value={note}
          ...
          <div className="overflow-y-auto flex-1 px-6 py-4">
            <div className="grid grid-cols-[1fr_120px_1fr_32px] gap-2 mb-2">
              <p className="text-[11px] text-foreground/30 font-semibold uppercase tracking-wider">Recipient (email/username)</p>
              <p className="text-[11px] text-foreground/30 font-semibold uppercase tracking-wider">Amount (GHS)</p>
              <p className="text-[11px] text-foreground/30 font-semibold uppercase tracking-wider">Note</p>
              <div />
            </div>
              ...
              {rows.map((row) => (
                <div key={row.id} className="grid grid-cols-[1fr_120px_1fr_32px] gap-2 items-center">
                  <input
                    ...
                    className="bg-black/30 border border-border rounded-xl px-3 py-2 text-sm text-foreground placeholder:text-foreground/20 focus:outline-none focus:border-[#B7EE7A]/50"
          ...
          <div className="px-6 py-4 border-t border-border flex-shrink-0">
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm text-foreground/50">
                ...
                <span className="text-foreground/30"> ({rows.length} recipients)</span>
              </p>
              <div className="flex gap-2">
                <button type="button" onClick={onClose} className="px-4 py-2.5 rounded-xl border border-border text-sm text-foreground/60 hover:text-foreground hover:bg-muted/30 transition-colors">
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={loading}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#174717] hover:bg-[#1e5e1e] text-sm font-semibold text-foreground transition-colors disabled:opacity-50"
                >
...
export default function BulkTransfersPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground">Bulk Transfers</h1>
          <p className="text-foreground/40 text-sm mt-0.5">Pay multiple recipients in one action</p>
        </div>
        <button
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 gap-3">
            <SendHorizonal size={32} className="text-foreground/15" />
            <p className="text-foreground/40 text-sm">No bulk transfers yet</p>
          ...
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    <th className="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-foreground/30">Transfer</th>
                    <th className="px-5 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-foreground/30">Total</th>
                    <th className="px-5 py-3 text-center text-[11px] font-semibold uppercase tracking-wider text-foreground/30 hidden md:table-cell">Recipients</th>
                ...
                <tbody className="divide-y divide-white/[0.04]">
                  {page.content.map((bt) => (
                    <tr key={bt.id} className="hover:bg-muted/10 transition-colors">
                      <td className="px-5 py-3.5">
                        <p className="text-xs font-mono text-foreground/50">{bt.id.slice(0, 8).toUpperCase()}</p>
                        {bt.note && <p className="text-xs text-foreground/40 mt-0.5 truncate max-w-xs">{bt.note}</p>}
                      </td>
                      <td className="px-5 py-3.5 text-right text-sm font-semibold text-foreground">{fmt(bt.totalAmount)}</td>
                      <td className="px-5 py-3.5 text-center text-xs text-foreground/50 hidden md:table-cell">
                        <span className="text-[#B7EE7A]">{bt.successCount}</span>
                        {bt.failureCount > 0 && <span className="text-red-400"> / {bt.failureCount} failed</span>}
                        <span className="text-foreground/30"> of {bt.recipientCount}</span>
                      </td>
                      <td className="px-5 py-3.5 text-center">
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-semibold ${STATUS_STYLE[bt.status] ?? "bg-muted/50 text-foreground/40"}`}>
                          {bt.status.replace(/_/g, " ")}
                        </span>
                      </td>
                      <td className="px-5 py-3.5 text-right text-xs text-foreground/40 hidden md:table-cell">{fmtDate(bt.createdAt)}</td>
                      <td className="px-5 py-3.5">
                        <button onClick={() => setSelectedId(bt.id)} className="p-1.5 rounded-lg text-foreground/30 hover:text-foreground hover:bg-muted/30 transition-colors" title="View detail">
            ...
            {page.totalPages > 1 && (
              <div className="flex items-center justify-between px-5 py-3 border-t border-border">
                <p className="text-xs text-foreground/30">{page.totalElements} batches · page {page.number + 1} of {page.totalPages}</p>
