      ...
      <div className="relative bg-[#1a1a1a] border border-white/10 rounded-2xl w-full max-w-md p-6">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs text-white/40 mb-1.5">Billing Interval</label>
              <select
                value={form.interval}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">Plans & Subscriptions</h1>
          <p className="text-white/40 text-sm mt-0.5">Recurring billing for your customers</p>
        </div>
        {tab === "plans" && (
          <button
      ...
      <div className="flex gap-1 bg-white/5 rounded-xl p-1 w-fit">
        {(["plans", "subscriptions"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors capitalize ${tab === t ? "bg-white/10 text-white" : "text-white/40 hover:text-white/70"}`}
          >
            {t}
          </button>
        ))}
                ...
                <div key={plan.id} className="px-5 py-4 flex items-center gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-white/50 mt-1">
                      <span className="font-semibold text-white">{fmt(plan.amount, plan.currency)}</span>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/5">
                      ...
                      <th className="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-white/30 hidden md:table-cell">Plan</th>
                      <th className="px-5 py-3 text-center text-[11px] font-semibold uppercase tracking-wider text-white/30">Status</th>
                      <th className="px-5 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-white/30 hidden md:table-cell">Next Billing</th>
                    ...
                    {subsPage.content.map((sub) => (
                      <tr key={sub.id} className="hover:bg-white/[0.02] transition-colors">
                        <td className="px-5 py-3.5">
                          <p className="text-sm font-medium text-white">{sub.customerName ?? "—"}</p>
                          {sub.customerEmail && <p className="text-xs text-white/40">{sub.customerEmail}</p>}
                        </td>
                        <td className="px-5 py-3.5 text-sm text-white/60 hidden md:table-cell">
                          {plans?.find((p) => p.id === sub.planId)?.name ?? sub.planId.slice(0, 8)}
                        </td>
                        <td className="px-5 py-3.5 text-center">
                          <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-semibold ${SUB_STATUS_STYLE[sub.status] ?? "bg-white/10 text-white/50"}`}>
                            {sub.status}
                          </span>
                        </td>
