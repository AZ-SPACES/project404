...
function Toggle({ enabled, onChange }: { enabled: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 flex-shrink-0 items-center rounded-full transition-colors focus:outline-none ${enabled ? "bg-[#174717]" : "bg-muted/50"}`}
    >
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-foreground/40 text-sm mt-0.5">Choose which events send you an email alert</p>
      </div>
      ...
      {prefs && GROUPS.map((group) => (
        <div key={group.label} className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border">
            ...
            <p className="text-xs text-foreground/35 mt-0.5">{group.description}</p>
          </div>
                ...
                <div>
                  ...
                  {key === "emailLowBalance" && prefs.emailLowBalance && (
                    <div className="flex items-center gap-2 mt-2">
                      <span className="text-xs text-foreground/40">Threshold (GHS):</span>
                      <input
                        type="number"
                        min="0.01"
                        step="0.01"
                        ...
                        className="w-28 bg-black/30 border border-border rounded-lg px-2.5 py-1 text-xs text-foreground placeholder:text-foreground/20 focus:outline-none focus:border-[#B7EE7A]/50"
                        placeholder="50.00"
                      />
                    </div>
                  )}
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        <div className="flex items-start gap-3">
          <Bell size={15} className="text-foreground/30 mt-0.5 flex-shrink-0" />
