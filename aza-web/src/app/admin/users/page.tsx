...
const STATUS_OPTS = ["", "ACTIVE", "SUSPENDED", "DEACTIVATED"];
const KYC_OPTS = ["", "NOT_STARTED", "PENDING", "UNDER_REVIEW", "VERIFIED", "REJECTED"];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-white/40 text-sm mt-1">
          {result ? `${result.totalElements.toLocaleString()} total` : ""}
        </p>
      </div>
      ...
      <form onSubmit={search} className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            type="text"
            value={query}
            onChange={e => { setQuery(e.target.value); if (!e.target.value) setPage(0); }}
        ...
        <select
          value={kycStatus}
          onChange={e => { setKycStatus(e.target.value); setPage(0); }}
          className="px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm text-white/70 focus:outline-none"
        >
          <option value="">All KYC</option>
          {KYC_OPTS.filter(Boolean).map(s => <option key={s} value={s}>{s.replace(/_/g, " ")}</option>)}
        </select>
      </form>
              ...
              <thead>
                <tr className="border-b border-white/5">
                  {["User", "Phone", "Account Status", "KYC", "Wallet", "Joined"].map(h => (
                    <th key={h} className="text-left text-xs text-white/30 font-medium uppercase tracking-wider px-4 py-3">
                      {h}
                    </th>
                  ))}
                  <th />
              ...
              <tbody className="divide-y divide-white/5">
                {result?.content.map(u => (
                  <tr key={u.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Badge value={u.accountStatus} map={STATUS_COLORS} />
                    </td>
                    <td className="px-4 py-3">
                      <Badge value={u.kycStatus} map={KYC_COLORS} />
                    </td>
