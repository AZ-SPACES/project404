...
const quickLinks = ['GitHub', 'Discord', 'Status', 'Blog'];
...
export function DeveloperMenu({ isOpen, onClose }: Props) {
  ...
  // Hover shape animations (mount once)
  useEffect(() => {
    if (!containerRef.current) return;

    // Initialize all shape elements invisible
          ...
          >
            <div className="dev-menu-grid" style={{ width: '100%', maxWidth: '1260px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,360px)', gap: 'clamp(32px,5vw,96px)', alignItems: 'center' }}>
              ...
              <div>
                <p
                  data-dev-fade
                  style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(183,238,122,0.55)', marginBottom: '20px' }}
                >
                  DEVELOPER TOOLS
                </p>
                  ...
                  {devLinks.map((l) => (
                    ...
                    >
                      <a
                        ...
                        onClick={onClose}
                        style={{ display: 'flex', flexDirection: 'column', padding: '13px 0', textDecoration: 'none', cursor: 'pointer' }}
                      >
              ...
              <div className="dev-menu-sidebar" style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                ...
                <div
                  data-dev-fade
                  style={{ background: 'rgba(183,238,122,0.05)', border: '1px solid rgba(183,238,122,0.14)', borderRadius: '16px', padding: '20px' }}
                >
...
{`curl -X POST \\
  api.aza.finance/v1/pay \\
  -H "Authorization: Bearer $KEY"`}
                  </pre>
                  <a href="#" style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', fontSize: '0.82rem', fontWeight: 600, color: '#B7EE7A', textDecoration: 'none' }}>
                    Read the docs →
                  </a>
                </div>
                ...
                <div data-dev-fade style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {quickLinks.map((label) => (
                    <a
                      key={label}
                      href="#"
                      style={{ padding: '6px 14px', borderRadius: '8px', fontSize: '0.78rem', fontWeight: 600, border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.55)', textDecoration: 'none', transition: 'border-color 0.15s, color 0.15s' }}
          ...
          <button
            ...
            style={{ position: 'absolute', top: '20px', right: '20px', zIndex: 20, width: '40px', height: '40px', borderRadius: '50%', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.7)', cursor: 'pointer' }}
          >
            <X size={18} />
          </button>
        </nav>
