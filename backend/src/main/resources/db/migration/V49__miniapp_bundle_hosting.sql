...
--
-- Each hosted app gets its own subdomain (<subdomain>.miniapps.aza.systems) rather
-- than a path on a shared host, so that one mini app cannot read another's
-- localStorage, cookies, IndexedDB or service workers. That isolation is the whole
-- reason for the column; it is not cosmetic.
--
...
-- DNS label: max 63 chars, lowercase alphanumeric + hyphen. NULL for EXTERNAL apps.
-- Note mini app ids allow underscores (e.g. "bolt_ghana") which are not valid in
-- hostnames, so the subdomain is derived and stored separately rather than reusing id.
ALTER TABLE mini_apps
...
-- Uploaded but not yet approved. Served at <subdomain>-preview.miniapps.aza.systems
-- for reviewers, and promoted to bundle_version on approval.
ALTER TABLE mini_apps
