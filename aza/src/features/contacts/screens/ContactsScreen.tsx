...
export default function ContactsScreen() {
  ...
  const [contactsList, setContactsList] = useState<Recipient[]>(INITIAL_RECIPIENTS);
        ...
        if (data.length > 0) {
          const deviceContacts: Recipient[] = data
            ...
            .map((c, index) => ({
              ...
              name: c.name,
              username: c.phoneNumbers?.[0]?.number || c.emails?.[0]?.email || "No contact info",
              avatar: c.image?.uri || `https://ui-avatars.com/api/?name=${encodeURIComponent(c.name)}&background=random`,
            }));
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity 
            ...
            onPress={async () => {
              try {
                await Share.share({
                  message: "Join me on Aza, the effortless way to send and receive money. Get the app now!",
                });
              } catch (error) {
        ...
        <View style={styles.searchRow}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => {
              Alert.alert(
                "Add Aza User",
                "You will soon be able to add other users by their @username or email.",
                [{ text: "OK" }]
              );
            }}
          >
