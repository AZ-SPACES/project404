  ...
  const handleCapture = async () => {
    try {
      if (cameraRef.current) {
        ...
        if (photo) {
          // Crop assuming the ID was placed perfectly inside the layout frame
          // Calculate the bounding box based on frame aspect ratio against picture dimension
          // Note: This is an approximation for Expo Go placeholder utility
          const pictureAspectRatio = photo.width / photo.height;
          const windowAspectRatio = width / height;
          
          let cropWidth, cropHeight, originX, originY;
          
          if (pictureAspectRatio > windowAspectRatio) {
            // Picture is wider than screen (relative)
            cropWidth = photo.width * 0.85;
            cropHeight = cropWidth * 0.63;
          } else {
            // Picture is taller
            cropWidth = photo.width * 0.85;
            cropHeight = cropWidth * 0.63;
